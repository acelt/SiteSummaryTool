...
SummaryTables <- function(EcoSitePlots, Species_plots_ecosite, 
                          SummaryVar, SummarizeBy){
...
TraceSpeciesCover <- Species_plots_ecosite[Species_plots_ecosite[["Species"]] %in% TraceCover_List,]
...
TraceCover_Table_SpList <- TraceSpeciesCover %>%
                ...
                UpdatedSpeciesCode, link) %>% unique()
...
if(SummaryVar== "Noxious" & SummarizeBy == "Plot"){
  ...
  table <- EcoSitePlots %>% dplyr::select(PlotID, PrimaryKey, AH_NoxCover, AH_NonNoxCover) %>%
