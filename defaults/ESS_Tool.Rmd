...
#devtools::install_github(repo = "https://github.com/acelt/SiteSummaryTool.git", ref = "dev")
...
if(BLM){
TerrADat_Path <- NULL
conn <- RODBC::odbcConnect("AIMPub",rows_at_time = 1)

TerrADat <- RODBC::sqlQuery(conn, 'SELECT * FROM ilmocAIMTerrestrialPub.ilmocAIMPubDBO.TerrestrialIndicators;')

Species_Indicator <- RODBC::sqlQuery(conn, 'SELECT * FROM ilmocAIMTerrestrialPub.ilmocAIMPubDBO.TerrestrialSpecies;')

SpeciesList <- RODBC::sqlQuery(conn, 'SELECT * FROM ilmocAIMTerrestrialPub.ILMOCAIMPUBDBO.tblStateSpecies;')
}
```
